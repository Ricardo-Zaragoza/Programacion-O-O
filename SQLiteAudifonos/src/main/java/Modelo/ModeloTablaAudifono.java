...
public class ModeloTablaAudifono implements TableModel {
    ...
    /*public boolean actualizarAudifono(Audifono audifono) throws SQLException {
        AudifonosDAO dao = new AudifonosDAO();
        return dao.update(audifono);
    }*/
    public boolean actualizarAudifono(Audifono audifono) {
        try {
            return ldao.update(audifono);
        } catch (SQLException sqle) {
            System.out.println(sqle.getMessage());
            return false;
        }
   ...
   /*public boolean eliminarAudifono(String id) {
       try {
           ...
           if (rowCount > 0) {
               // Actualizar los IDs de todos los registros restantes
               String sqlUpdateIDs = "UPDATE audifonos SET id = id -1;";
               PreparedStatement pstmUpdateIDs = Conexion.getInstance("audifonosDB.db").getConnection().prepareStatement(sqlUpdateIDs);
               pstmUpdateIDs.executeUpdate();
               // Cargar los datos actualizados desde la base de datos
               datos = ldao.obtenerTodo();
               return true;
           } else {
               return false;
